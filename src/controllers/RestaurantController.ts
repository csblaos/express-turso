...
export class RestaurantController {
	...
	static send = SyncFunction.handler(async (req: Request,res: Response)=>SuccessHandler.send(res,req.requestId,{data:await RestaurantInterface.sendRound(storeId(req),String(req.params.id),Number(req.body.expected_version),String(req.header("Idempotency-Key")||""),actor(req))}));
