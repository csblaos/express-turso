...
export class PosController {
	...
	static checkout = SyncFunction.handler(async (req: Request, res: Response) => {
		...
		const data = await OrderInterface.checkout({
			...
			payment_currency: typeof body.payment_currency === "string" ? body.payment_currency : null,
			amount_tendered_foreign: body.amount_tendered_foreign == null ? null : Number(body.amount_tendered_foreign),
			expected_exchange_rate: body.expected_exchange_rate == null ? null : Number(body.expected_exchange_rate),
