...
export default class RestaurantValidator extends ValidatorMiddleware {
	...
	static readonly station = RestaurantValidator.init({ body: z.object({ store_id: z.string().optional(), name: z.string().trim().min(1).max(80), sort_order: z.coerce.number().int().optional(), is_active: z.union([z.boolean(), z.coerce.number().int().min(0).max(1)]).optional(), category_ids: z.array(z.string().trim().min(1)).max(200).optional() }) });
	...
	static readonly cancel = RestaurantValidator.init({ body: z.object({ store_id: z.string().optional(), expected_version: version, qty: z.coerce.number().int().nonnegative().optional(), reason: z.string().trim().max(280).optional() }) });
	...
	static readonly checkout = RestaurantValidator.init({ body: z.object({ store_id: z.string().optional(), expected_version: version, payment_method: z.enum(["cash", "qr_transfer", "credit_card"]), payment_account_id: z.string().trim().nullable().optional(), dispatch_mode: z.enum(["existing", "direct"]).optional(), amount_tendered: z.coerce.number().nonnegative().nullable().optional(), payment_currency: z.enum([ "LAK", "THB", "USD" ]).nullable().optional(), amount_tendered_foreign: z.coerce.number().nonnegative().nullable().optional(), expected_exchange_rate: z.coerce.number().positive().nullable().optional(), payment_reference: z.string().trim().max(160).nullable().optional(), note: z.string().trim().max(500).nullable().optional() }) });
	static readonly routingQuery = RestaurantValidator.init({ query: store.extend({ search: z.string().trim().max(80).optional() }) });
	// null is meaningful on a product: it hands the decision back to the category.
	static readonly kitchenFlag = RestaurantValidator.init({ body: z.object({ store_id: z.string().optional(), send_to_kitchen: z.union([ z.boolean(), z.coerce.number().int().min(0).max(1) ]).nullable().optional() }) });
