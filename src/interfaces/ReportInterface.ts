...
export class ReportInterface{
	...
	private static async ensureIndexes(){await StoreInterface.ensureColumns();if(this.initialized)return;const db=DbConn.getClient();await db.batch([
	...
	static async dashboard(storeId:string,input:ReportPeriodInput):Promise<any>{
		...
		return{currency:String(row(results[11]).currency||"LAK"),preset:periods.preset,timezone_offset:input.timezoneOffset,period:periods.current,comparison_period:periods.previous,generated_at:new Date().toISOString(),
			...
			profitability:{revenue,known_cost_revenue:knownRevenue,known_cost:knownCost,known_gross_profit:grossProfit,gross_margin_percent:knownRevenue?grossProfit/knownRevenue*100:0,cost_coverage_percent:revenue?knownRevenue/revenue*100:0,unknown_cost_revenue:unknownRevenue,unknown_cost_bills:number(profit.unknown_cost_bills),bill_count:bills},sales_series:series,
			...
			operational_signals:{peak_period:peak?.label||null,peak_revenue:peak?.revenue||0,primary_payment_method:paymentRows[0]?String(paymentRows[0].method):null,primary_payment_percent:paymentRows[0]&&paymentTotal?number(paymentRows[0].amount)/paymentTotal*100:0,restock_sku_count:lowStock.length,out_of_stock_count:number(inventorySignals.out_of_stock_count),negative_stock_count:number(inventorySignals.negative_stock_count),no_sales_count:number(inventorySignals.no_sales_count),inventory_value:number(inventorySignals.inventory_value)}};
	}
	...
	static async purchasing(storeId:string,input:ReportPeriodInput):Promise<any>{
		...
		const results=await db.batch([
			...
			{ sql:"SELECT COALESCE(SUM(ics.qty_base_on_hand*ics.average_unit_cost_base),0) inventory_value,COUNT(*) product_count FROM inventory_cost_summaries ics WHERE ics.store_id=? AND ics.qty_base_on_hand>0",args:[ storeId ] as InValue[] },
