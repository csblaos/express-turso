...
const draftItems = computed(() => order.value?.items.filter((item) => item.line_status === "draft") || []);
const cartItemCount = computed(() => (order.value?.items || [])
...
const printItems = computed(() => printKind.value === "kitchen"
...
async function addProduct(product: Product) {
	if (product.stock_state === "inactive" || actionPending.value) return;
	...
	try {
		if (!order.value) {
			...
			const response = await apiFetch<Envelope<Order>>("/pos/restaurant/orders", {
				...
				body: { store_id: props.storeId, service_mode: "pickup", initial_item: { product_id: product.id, qty: 1 } },
			});
			order.value = response.data;
			createKey = "";
			toast.success({ title: t("restaurantPos.queueOpened", { queue: response.data.queue_no || "-" }) });
		} else {
			const response = await apiFetch<Envelope<Order>>(`/pos/restaurant/orders/${order.value.id}/items`, {
				method: "POST", body: { store_id: props.storeId, product_id: product.id, qty: 1, expected_version: order.value.version },
			});
			order.value = response.data;
		}
	} catch (error) {
		toast.error({ title: t("restaurantPos.addFailed"), description: localizedApiError(error) });
		if (order.value) await refreshOrder();
...
async function changeQty(item: OrderItem, delta: number) {
	...
	try {
		...
		order.value = response.data;
	} catch (error) { toast.error({ title: "แก้จำนวนไม่สำเร็จ", description: localizedApiError(error) }); await refreshOrder(); }
	finally { actionPending.value = false; }
}
...
async function removeItem(item: OrderItem) {
	...
	try {
		...
		order.value = response.data;
	} catch (error) { toast.error({ title: "ลบรายการไม่สำเร็จ", description: localizedApiError(error) }); await refreshOrder(); }
	finally { actionPending.value = false; }
}
...
async function sendKitchen(options: { print: boolean; park: boolean; pay: boolean }) {
	...
	try {
		...
		const response = await apiFetch<Envelope<Order>>(`/pos/restaurant/orders/${order.value.id}/send`, {
			method: "POST", headers: { "Idempotency-Key": sendKey }, body: { store_id: props.storeId, expected_version: order.value.version },
		});
		order.value = response.data;
		sendKey = "";
	...
	} catch (error) { toast.error({ title: "ส่งครัวไม่สำเร็จ", description: localizedApiError(error) }); await refreshOrder(); }
	finally { actionPending.value = false; }
}
...
function openCheckout(dispatch: "existing" | "direct") {
	...
	cashTendered.value = Number(order.value.total || 0);
...
async function checkout() {
	...
	} catch (error) { toast.error({ title: "ชำระเงินไม่สำเร็จ", description: localizedApiError(error) }); }
	finally { actionPending.value = false; }
}
...
<template>
	...
	<AppResponsivePanel v-model="checkoutPanel" title="ชำระเงิน" :description="orderLabel" desktop-width="600px"><div class="space-y-4"><div v-if="checkoutDispatch === 'direct'" class="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">ระบบจะตัดสต็อกและปิดออเดอร์ทันที โดยไม่ส่งหรือพิมพ์ใบครัว</div><div class="grid grid-cols-3 gap-2"><AppButton v-for="method in [{ id: 'cash', label: 'เงินสด' }, { id: 'qr_transfer', label: 'QR / โอน' }, { id: 'credit_card', label: 'บัตร' }]" :key="method.id" :color="paymentMethod === method.id ? 'primary' : 'neutral'" :variant="paymentMethod === method.id ? 'solid' : 'soft'" @click="paymentMethod = method.id as any">{{ method.label }}</AppButton></div><UFormField v-if="paymentMethod === 'cash'" label="รับเงิน"><UInput v-model.number="cashTendered" class="w-full" type="number" min="0" /></UFormField><div class="rounded-md bg-neutral-50 p-4"><div class="flex justify-between"><span>ยอดชำระ</span><strong>{{ money(order?.total || 0) }}</strong></div><div v-if="paymentMethod === 'cash'" class="mt-2 flex justify-between text-sm"><span>เงินทอน</span><span>{{ money(Math.max(0, cashTendered - (order?.total || 0))) }}</span></div></div><AppButton block :loading="actionPending" :disabled="paymentMethod === 'cash' && cashTendered < (order?.total || 0)" @click="checkout">ยืนยันชำระเงิน</AppButton></div></AppResponsivePanel>
	...
	<div class="restaurant-print-root"><div class="print-sheet"><h1>{{ printKind === 'kitchen' ? 'ใบสั่งครัว' : printKind === 'check' ? 'ใบเช็กบิล' : 'ใบเสร็จรับเงิน' }}</h1><p v-if="printKind === 'check'" class="print-unpaid">ยังไม่ชำระ</p><p>{{ orderLabel }}<template v-if="order?.service_mode === 'pickup'"> · ซื้อกลับบ้าน</template></p><p>{{ order?.order_no }}<template v-if="printRound"> · รอบ {{ printRound }}</template></p><hr><div v-for="item in printItems" :key="item.id"><div class="print-line"><span>{{ item.name }} <b v-if="item.is_gift">(ฟรี)</b></span><span>× {{ item.qty }}<template v-if="printKind !== 'kitchen'"> · {{ money(item.line_total) }}</template></span></div><p v-if="item.note" class="print-note">หมายเหตุ: {{ item.note }}</p></div><template v-if="printKind !== 'kitchen'"><hr><div class="print-line"><span>ยอดก่อนภาษี</span><span>{{ money(order?.subtotal || 0) }}</span></div><div v-if="order?.vat_amount" class="print-line"><span>ภาษี</span><span>{{ money(order.vat_amount) }}</span></div><div class="print-total"><strong>รวม</strong><strong>{{ money(order?.total || 0) }}</strong></div><template v-if="printKind === 'receipt'"><div class="print-line"><span>วิธีชำระ</span><span>{{ order?.payment_method }}</span></div><div class="print-line"><span>รับเงิน</span><span>{{ money(order?.amount_tendered || 0) }}</span></div><div class="print-line"><span>เงินทอน</span><span>{{ money(order?.change_amount || 0) }}</span></div></template></template><p class="print-time">{{ new Date().toLocaleString('th-TH') }}</p></div></div>
</template>
