...
<template><div class="relative w-full" :style="{height}"><div v-if="empty" class="absolute inset-0 grid place-items-center rounded-md bg-stone-50 text-sm text-stone-500"><slot name="empty">{{t('reportPage.noData')}}</slot></div><div ref="target" class="h-full w-full" /></div></template>
