...
function mapApiProduct(
	...
	return {
		...
		location: product.location?.trim() ? product.location.trim() : "-",
...
function normalizeLocationInput(value: string | number) {
	const normalized = String(value ?? "").toUpperCase();
	return normalized.length > 80 ? normalized.slice(0, 80) : normalized;
}

function handleProductLocationInput(value: string | number) {
	const normalized = normalizeLocationInput(value);
	if (productForm.location !== normalized) {
		productForm.location = normalized;
	}
}

function handleProductEditLocationInput(value: string | number) {
	const normalized = normalizeLocationInput(value);
	if (productEditForm.location !== normalized) {
		productEditForm.location = normalized;
	}
}
...
function removeDraftSaleUnit(draftId: string) {
	...
	if (validationError) {
		...
		const payload = {
			...
			location: productForm.location.trim().toUpperCase() || null,
...
async function saveProductEdit() {
	...
	const payload: Record<string, unknown> = {
		...
		location: productEditForm.location.trim().toUpperCase() || null,
...
async function printSelectedProductBarcode() {
	...
	function clearProductEditDeepLinkQuery() {
		const nextQuery = { ...route.query } as Record<string, unknown>;
		delete nextQuery.edit_product_id;
		delete nextQuery.focus;
		router.replace({ query: nextQuery });
	}

	async function tryHandleProductEditDeepLink() {
		const productId = readQueryString(route.query.edit_product_id);
		if (!productId) return;
		if (productsPending.value) return;

		if (!products.value.some((product) => product.id === productId)) {
			appToast.error({
				title: "ไม่พบสินค้า",
				description: "โปรดลองเปิดใหม่จากหน้า Inventory อีกครั้ง",
				timeout: 2800,
			});
		...
		if (canUpdateProduct.value) {
			...
			const focus = readQueryString(route.query.focus);
			if (focus === "location") {
				const input = productEditLocationInputRef.value?.input;
				input?.focus();
				input?.select();
			}
		} else {
										...
										<div class="flex flex-wrap items-start justify-between gap-2">
											<div class="min-w-0">
												...
												<div class="mt-1 flex min-w-0 items-center gap-2">
														<p class="min-w-0 flex-1 truncate text-sm text-stone-500">
															{{ selectedProduct.sku }} · {{ selectedProduct.barcode }}<template v-if="selectedProduct.location !== '-'"> · {{ selectedProduct.location }}</template>
														</p>
													<AppButton
														color="secondary"
														variant="soft"
