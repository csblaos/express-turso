...
const activeMode = ref<"all" | "linked" | "sharing" | "missing-code" | "missing-address">("all");
...
const summary = ref<BranchListResponse["summary"]>({
	branches_total: 0,
	stores_covered: 0,
	linked_branches: 0,
	custom_sharing_branches: 0,
	missing_code: 0,
	missing_address: 0,
});
...
const pageSummaryText = computed(() => (
	totalItems.value === 0
		? "ยังไม่มีข้อมูล"
		: `${pageStart.value}-${pageEnd.value} จาก ${totalItems.value} สาขา`
));

const overviewStats = computed(() => ([
	{ label: "Branches", value: summary.value.branches_total, note: "สาขาทั้งหมดใต้ superadmin นี้" },
	{ label: "Stores covered", value: summary.value.stores_covered, note: "จำนวนร้านที่มีสาขาอย่างน้อย 1 แห่ง" },
	{ label: "Linked source", value: summary.value.linked_branches, note: "สาขาที่มี source branch ผูกไว้" },
	{ label: "Sharing mode", value: summary.value.custom_sharing_branches, note: "สาขาที่มี sharing mode กำหนดไว้" },
	{ label: "Missing code", value: summary.value.missing_code, note: "สาขาที่ยังไม่มี branch code" },
	{ label: "Missing address", value: summary.value.missing_address, note: "สาขาที่ยังไม่มีที่อยู่" },
	]));

function resolveApiErrorMessage(errorValue: unknown, fallback = "โหลด branch config ไม่สำเร็จ") {
...
function sharingLabel(item: BranchRecord) {
	if (item.sharing_mode) return item.sharing_mode;
	if (item.source_branch_id) return "linked";
	return "standalone";
}

function sharingTone(item: BranchRecord) {
	if (item.sharing_mode) return "primary";
	if (item.source_branch_id) return "warning";
	return "neutral";
}
...
async function loadBranches() {
	...
	try {
		...
		const response = await apiFetch<ApiEnvelope<BranchListResponse>>(`/superadmin/branches?${query.toString()}`);
						...
						<div class="flex h-full min-h-0 flex-col">
								...
								<div class="grid gap-2.5 sm:grid-cols-[minmax(0,1fr)_180px_auto]">
									<div class="relative">
										...
										<input
											v-model="searchQuery"
											type="search"
											placeholder="ค้นหาสาขา ร้าน code หรือที่อยู่"
							...
							<div ref="listScrollRef" class="scrollbar-soft min-h-0 flex-1 overflow-y-auto pb-[calc(4rem+env(safe-area-inset-bottom))]">
								...
								<div v-else>
									...
									<div v-if="!branches.length" class="p-5 text-center text-sm text-stone-500">
										ยังไม่มีสาขาใน scope นี้
									</div>
										...
										>
											<div class="flex items-start justify-between gap-3">
												<div class="min-w-0">
													<div class="flex flex-wrap items-center gap-2">
														<p class="truncate text-sm font-semibold text-stone-900">{{ branch.name }}</p>
														<UBadge :color="sharingTone(branch)" variant="soft" :label="sharingLabel(branch)" />
													</div>
													<p class="mt-1 truncate text-xs text-stone-500">{{ branch.store_name }}</p>
													<p class="mt-2 text-xs text-stone-500">สร้างเมื่อ {{ formatDateTime(branch.created_at) }}</p>
												</div>
											...
											<div class="mt-3 grid gap-2 sm:grid-cols-3">
												<div class="rounded-md bg-neutral-50 px-3 py-2.5">
													<p class="text-[11px] uppercase tracking-[0.14em] text-stone-400">Address</p>
													<p class="mt-1 text-sm font-semibold text-stone-900">{{ branch.address || "ยังไม่มี" }}</p>
												</div>
												<div class="rounded-md bg-neutral-50 px-3 py-2.5">
													<p class="text-[11px] uppercase tracking-[0.14em] text-stone-400">Source branch</p>
													<p class="mt-1 text-sm font-semibold text-stone-900">{{ branch.source_branch_id || "none" }}</p>
												</div>
												<div class="rounded-md bg-neutral-50 px-3 py-2.5">
													<p class="text-[11px] uppercase tracking-[0.14em] text-stone-400">Sharing config</p>
													<p class="mt-1 text-sm font-semibold text-stone-900">{{ branch.sharing_config ? "configured" : "default" }}</p>
												</div>
											</div>
										</button>
									</template>
