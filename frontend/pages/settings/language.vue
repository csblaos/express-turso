...
const languageOptions: LanguageOption[] = [
	{
		id: "th",
		flagSvg: `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 32 32">
...
watch(currentLanguage, (value) => {
	draftLanguage.value = value;
});

const selectedOption = computed(() => (
	languageOptions.find((option) => option.id === draftLanguage.value) || languageOptions[0]
));
...
const previewNumber = computed(() => {
	try {
		return new Intl.NumberFormat(selectedOption.value.locale).format(1234567.89);
	} catch {
		return "-";
	}
});

const hasChanges = computed(() => draftLanguage.value !== currentLanguage.value);
	...
	>
		<template #default="{ openSidebar }">
			<div class="grid gap-3 pb-[calc(5.75rem+env(safe-area-inset-bottom))] lg:gap-4 lg:pb-3">
				<div class="hidden md:block">
					...
					<UCard class="rounded-none border-0 bg-white shadow-[0_8px_24px_rgba(31,28,24,0.06)] ring-1 ring-neutral-200 sm:rounded-md">
						<div class="grid grid-cols-3 gap-2 p-0">
							...
							<div class="min-w-0 rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-center">
								<p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">Locale</p>
								<p class="mt-1 truncate text-base font-semibold text-stone-950">{{ selectedOption.locale }}</p>
							</div>
							<div class="min-w-0 rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2 text-center">
				...
				<div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.42fr)] lg:items-start lg:gap-4">
						...
						<div class="flex flex-col">
							...
							<div class="space-y-3 px-4 py-4">
								<div class="rounded-md border border-neutral-200 bg-neutral-50 px-4 py-3">
									<p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">Date/Time</p>
									<p class="mt-1 text-sm font-semibold text-stone-950">{{ previewDate }}</p>
								</div>
								<div class="rounded-md border border-neutral-200 bg-neutral-50 px-4 py-3">
									<p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">Number</p>
									<p class="mt-1 text-sm font-semibold text-stone-950 tabular-nums">{{ previewNumber }}</p>
								</div>
								<div class="rounded-md border border-neutral-200 bg-neutral-50 px-4 py-3">
									<p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-stone-500">Selected</p>
									<p class="mt-1 flex items-center justify-center gap-2 text-sm font-semibold text-stone-950">
										<span class="inline-flex h-5 w-5 items-center justify-center" aria-hidden="true" v-html="selectedFlagSvg" />
